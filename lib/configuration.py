...
class ModelAndLoss(nn.Module):
    def __init__(self, args, model, training_loss, evaluation_loss=None):
        ...
        self.rewarp = hasattr(args, 'rewarp') and args.rewarp
    ...
    def aggregate_tensor(self, tensors, mode='mean'):
        stacked = torch.stack(tensors)
        if mode == 'min':
            stacked_min = stacked.abs().min(dim=0)[1]
            stacked_ix = torch.stack([(1 - stacked_min), stacked_min])
            return (stacked * stacked_ix.float()).sum(dim=0)
        elif mode == 'max':
            stacked_max = stacked.abs().max(dim=0)[1]
            stacked_ix = torch.stack([(1 - stacked_max), stacked_max])
            return (stacked * stacked_ix.float()).sum(dim=0)
        else:
            return stacked.mean(dim=0)

    def aggregate_results(self, output_dicts, mode='mean'):
        ...
        for key in output_dicts[0].keys():
            if mode == 'mean':
                output_dict[key] = self.aggregate_tensor([o[key] for o in output_dicts])
        return output_dict
    ...
    def forward(self, example_dict):
        ...
        # -------------------------------------
        output_dicts = []
        if type(self.model) == nn.ModuleList:
            for m in self._model.module:
                output_dicts.append(m(example_dict))
            output_dict = self.aggregate_results(output_dicts)
        else:
        ...
        if self.training:
            ...
            if 'flo_res' in example_dict:
                print("Basename {}".format(example_dict['full_basename']))
                print("loaded size {}".format(example_dict['flo_res'].shape))
                print("output size {}".format(output_dict['flow'].shape))
                if 'target1' in example_dict:
                    print("target size {}".format(example_dict['target1'].shape))
                output_dict['flow'] = example_dict['flo_res'].cuda()
...
def configure_model_and_loss(args):
    ...
    with logger.LoggingBlock("Model and Loss", emph=True):
        ...
        if type(args.checkpoint) == list and len(args.checkpoint) > 1:
            models = nn.ModuleList([tools.instance_from_kwargs(args.model_class, kwargs)
        ...
        # Model and loss
        # ----------------------------------------------------
        model_and_loss = ModelAndLoss(args, models, training_loss, validation_loss)
...
class CheckpointSaver:
    ...
    def _load_state_dict_into_module(self, state_dict, module, strict=False,
                                     avoid_list=[], debug=False):
        own_state = module.state_dict()
        own_state = {name.replace('.module', ''): v for name, v in own_state.items()}
        state_dict = {name.replace('.module', ''): v for name, v in state_dict.items()}

        orig_keys = state_dict.keys()
        for avoid in avoid_list:
            state_dict = {k: v for k, v in state_dict.items() if avoid not in k.lower()}
        avoided_keys = orig_keys - state_dict.keys()
        if debug:
            print("Avoided: {}".format(avoided_keys))
        ...
        for name, param in state_dict.items():
            if name in own_state:
                ...
                raise KeyError('unexpected key "{}" in state_dict'
                               .format(name))
        missing = set(own_state.keys()) - set(state_dict.keys()) - avoided_keys
        my_missings = set(state_dict.keys()) - set(own_state.keys()) - avoided_keys
        if strict:
            if len(missing) > 0:
                raise KeyError('missing keys in state_dict: "{}"'.format(missing))
        elif missing or my_missings:
            print('missing keys in state_dict: "{}"'.format(missing))
            print('missing keys in own dict: "{}"'.format(my_missings))
    ...
    def restore(self, filename, model_and_loss, include_params="*", exclude_params=()):
        ...
        if hasattr(model_and_loss.model, 'avoid_list'):
            avoid_list = model_and_loss.model.avoid_list
        else:
            avoid_list = []
...
def configure_checkpoint_saver(args, model_and_loss):
    if type(args.checkpoint) == list and len(args.checkpoint) > 1:
        return load_checkpoints_savers(args, model_and_loss)
    else:
        return load_checkpoint_saver(args, model_and_loss)


def load_checkpoints_savers(args, model_and_loss):
    checkpoints = args.checkpoint
    models = model_and_loss.model
    outs = []
    for ix in range(len(checkpoints)):
        args.checkpoint = checkpoints[ix]
        model_and_loss._model = models[ix]
        outs.append(load_checkpoint_saver(args, model_and_loss))
        models[ix] = model_and_loss.model
    model_and_loss._model = models
